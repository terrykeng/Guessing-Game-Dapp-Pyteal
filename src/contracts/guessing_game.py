...
def create_challenge(rand: abi.String, payment: abi.PaymentTransaction):
        return Seq(
            ...
            Assert(
                payment.get().sender() == App.globalGet(Bytes("player1")),
            ),
...
def play_value(p: abi.String):
        return Seq(
            Assert(
                ...
                If(Txn.sender() == App.globalGet(Bytes("player1")))
                .Then(
                    App.localPut(Txn.sender(), l_p, p.get()),
                  
                )
                .ElseIf((Txn.sender() == App.globalGet(Bytes("player2"))))
                .Then(
                    App.localPut(Txn.sender(), l_p, p.get()),
...
def winner(play1: Expr, play2: Expr):
        #v = App.globalGet(seed),
        return Seq(
            Return(Cond(
                [And(Base64Decode.std(play1) == Base64Decode.std(App.globalGet(seed)),Base64Decode.std(play2) == Base64Decode.std(App.globalGet(seed)) ), Int(3)],
                [And(Base64Decode.std(play1) == Base64Decode.std(App.globalGet(seed)), Base64Decode.std(play2) != Base64Decode.std(App.globalGet(seed))), Int(1)],
                [And(Base64Decode.std(play1) != Base64Decode.std(App.globalGet(seed)), Base64Decode.std(play2) == Base64Decode.std(App.globalGet(seed)) ), Int(2)],
                ))
...
def reveal(account1:abi.Account, account2: abi.Account, *, output: abi.Uint64 ):
    Assert(
        Txn.sender() == App.globalGet(Bytes("player1")),
    ),
...
def reset_all(acc1 : Expr , acc2 : Expr):
    return Seq(App.localPut(acc1 , l_p , Bytes("")),
